...
- **Local variable limitation.** Calls through local variables (e.g., `var x = service.GetFoo(); x.Bar()`) may go undetected when the return type cannot be inferred. DI-injected fields, C# and TypeScript typed parameters (including null/optional chaining), `this` / `base` calls, direct receivers, and ternary/nullish-coalescing receivers whose branches resolve to the same type are fully supported.
