...
6. **Self-review for hidden bugs** — BEFORE documenting, critically review ALL changes in this branch:
   ...
   - **FULL FLOW REVIEW**: for each modified function, trace ALL callers up to the public API entry point. Check: (1) Are there other callers that now receive different data? (2) Does the change affect downstream consumers (JSON field additions/removals, error type changes)? (3) Are parallel code paths (multi-method batch, different directions) also affected? (4) Is the new behavior consistent with equivalent logic in sibling functions (same comparison style, same case handling)? This step catches regressions that unit tests miss because they test functions in isolation, not in the full dispatch pipeline.
...
7. **E2E test plan** — the appropriate file in `docs/e2e/` is updated with test scenarios for the change (see `docs/e2e/README.md` for the file index)
...
9. **CLI & MCP discoverability** — for every new feature:
  ...
  - LLM instructions: verify `xray_help` output covers the new capability
...
| 5 | `docs/e2e/*.md` | Test scenario (in the appropriate module file) | Always |
...
- **Testing is mandatory** — every code change MUST include:
  ...
  - **E2E test plan update** (appropriate file in `docs/e2e/`) with a test scenario for the change
...
4. **Assess action items**: Are there real problems with xray code, or is this just expected behavior?
