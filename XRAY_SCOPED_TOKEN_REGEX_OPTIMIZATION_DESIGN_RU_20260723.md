...
- TF-IDF scores;
- ranking;
- numerator `termsMatched`; denominator следует execution universe, указанной в `regexExpansion.accountingScope`, и может различаться между global и scoped strategy;
- OR/AND behavior;
- invert complement;
- count-only totals;
...
Search results, ranking, occurrences и raw query representation остаются backward-compatible. Numerator `termsMatched` остается result semantics и обязан совпадать между strategies. Denominator является execution accounting: для `globalVocabulary` это global matching token count, для `resolvedFiles` — scoped matching token count. Поэтому denominator может различаться при наличии global matching tokens вне scope. Клиент обязан интерпретировать его вместе с `accountingScope`; scoped planner не выполняет global scan только ради старого denominator, потому что это уничтожило бы optimization. Differential tests строго сравнивают result payload после нормализации только denominator и отдельно доказывают его равенство `regexExpansion.matchedTokenCount` каждой strategy.
...
Criterion сравнил `scoped_reference_ratio` 10%, 25% и 50% на T=1k/10k/50k, K=1%/10%, selectivity=1%/100% и low/medium/hot sharing. Выбран measured default 50%: scoped execution оставался neutral или быстрее до этой границы, тогда как 10% и 25% преждевременно выбирали multi-millisecond global fallback для broad regex. Порог остается internal planner policy, а не публичным контрактом.
