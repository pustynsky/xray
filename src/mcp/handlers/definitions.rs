...
fn hint_unsupported_extension(
    ...
    for term in &terms {
        if let Some(dot_pos) = term.rfind('.') {
            ...
            // (e.g., no containsLine or name filter was specified). Delegate the UX
            // phrasing to the `xml_on_demand` module so the copy lives in one place.
            #[cfg(feature = "lang-xml")]
            if crate::definitions::parser_xml::is_xml_extension(&ext_lower) {
                return Some(super::xml_on_demand::hint_for_xml_extension(&ext_lower));
            }
