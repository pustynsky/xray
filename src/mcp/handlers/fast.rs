...
/// so the trailing wildcard is open-ended: `Invoice?_V?` matches
/// `Invoices_V2.sql`, `Order*` matches `Order123.cs`. Without this
...
mod fast_unit_tests {
    ...
    fn test_glob_trailing_wildcard_drops_end_anchor() {
        ...
        assert!(glob_matches("Invoice?_V?", "Invoices_V2.sql"));
