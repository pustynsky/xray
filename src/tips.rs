...
pub fn parameter_examples(def_extensions: &[String]) -> Value {
    ...
    json!({
        ...
        "xray_edit": {
            ...
            "_responseFields_syncReindex": "After a real write (NOT dryRun), response includes: contentIndexUpdated (bool), defIndexUpdated (bool), fileListInvalidated (bool — true on file creation), reindexElapsedMs (string, e.g. '0.42'), skippedReason (string: 'outsideServerDir' | 'extensionNotIndexed' | 'insideGitDir' — file is still written, only index update skipped), reindexWarning (only set on lock poisoning — FS watcher reconciles within 500ms), fileCreated: true (on new files). dryRun: true OMITS all reindex fields. Multi-file: per-file reindex outcome + summary.reindexElapsedMs (one batched call).",
