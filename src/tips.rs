...
pub fn parameter_examples(def_extensions: &[String]) -> Value {
    ...
    json!({
        ...
        "xray_callers": {
            ...
            "resolveInterfaces": "false keeps direct interface nodes only; true also expands callers/callees through implementing types. Applies at every tree depth and is echoed as query.resolveInterfaces. Default: true",
        ...
        "xray_edit": {
            ...
            "_responseFields_syncReindex": "After a non-dryRun request, response includes: writeStatus ('committed' | 'unchanged'; unchanged means final bytes matched and no write occurred), reindexStatus ('completed' | 'skipped'), reindexSkipReason (string: 'contentUnchanged' | 'outsideServerDir' | 'extensionNotIndexed' | 'insideGitDir' | 'ignoredByIndexRules'; contentUnchanged means no write or index update occurred, while other skip reasons mean the file was written but index update was out of scope), contentIndexUpdated (bool), defIndexUpdated (bool), fileListInvalidated (bool — true on file creation), reindexElapsedMs (string, e.g. '0.42'), reindexWarning (only set on lock poisoning — FS watcher reconciles within 500ms), fileCreated: true (on new files), skippedReason (DEPRECATED alias of reindexSkipReason — use reindexStatus + reindexSkipReason instead). dryRun: true OMITS all reindex fields. Multi-file: per-file reindex outcome + summary.reindexElapsedMs (one batched call).",
            "_resultFidelity": "Files over 2,000 lines return bounded real hunks that are not patch-applicable; any rendered diff over 256 KB is truncated. Mixed LF/CRLF files are rejected before dryRun or write; pure LF and CRLF are preserved.",
            ...
            "expectedMatchCount": "Per-edit safety check: required applied replacement/insert count; skipped edits count 0.",
            ...
            "allowGitInternals": "Explicitly allow edits inside .git internals (default: false). The write may proceed, but reindex remains skipped with reason insideGitDir.",
