...
- **Preserved monotonic definition generations during server startup.** Definition-index cache loads and background rebuilds now publish strictly above both the persisted and current in-memory generation, so MCP `definitionGeneration` telemetry cannot move backward when watcher or `xray_edit` updates race startup publication.

- **Made Angular definition updates atomic across TypeScript and owned HTML inputs.** The effective definition-index upgrade for this release is v7 to v11; v11 persists a monotonic definition generation plus `(size, mtime, SHA-256)` input fingerprints. Watcher batches, periodic recovery, and synchronous `xray_edit` reindexing now prepare and validate TypeScript/TSX component records and external-template contributions outside the write lock, retry one raced snapshot, then publish selectors, owners, forward/reverse template edges, source states, and fingerprints under one lock and one generation bump. Transient source-read races preserve the previous graph and requeue only unstable paths while stable batch peers continue; a runtime pending registry retries paths independently of the global watermark and path-aware tombstoning and runtime quarantine end a persistent failure after three batches until a new edit event reactivates the path. A second synchronous conflict is reported as `pending` with per-path update telemetry. Owned external HTML is an auxiliary definition input even when `html` is absent from `--ext`; edit/delete/recreate, selector changes, component removal, and combined template/metadata renames become visible immediately without a full rebuild. Definition indexes rebuild automatically after upgrade.

- **Angular template callers now resolve class and selector roots exactly and honor caller scope.** Upward class queries use the component selector, downward selector queries resolve the matching component root, ambiguous in-scope roots return `ambiguous_template_root` by default, and `productionOnly`, `excludeDir`, `excludeFile`, and `ext` filter roots and recursive template nodes before ambiguity, pagination, and totals. Responses expose the chosen class/selector precedence through `query.templateRootInterpretation`. Fully filtered component roots report `template_root_out_of_scope` unless the same text resolves as a selector, in which case `templateRootInterpretation="selector"` reports the fallback; explicit `ambiguityPolicy="legacy"` retains fan-out and marks it with `summary.legacyAmbiguousRootFanOut`.

- **Angular component metadata is now AST-structured and supports static inline templates.** Definition indexing now persists explicit inline, external, unavailable-external, dynamic, and missing template states in `angular_components`; full builds derive selector, normalized template-owner, child, and reverse-parent indexes from those records. Inline and external templates use the same active-context tokenizer, duplicate selectors return ambiguity metadata instead of first-match resolution, and dynamic/missing/unavailable/unresolved states make template navigation partial via `summary.templateResolutionReasons` (`ambiguous_template_selector`, `unresolved_template_selector`, `dynamic_template_index_blind_spot`, `component_record_unavailable`, `dynamic_component_selector`, `missing_component_selector`, `dynamic_template`, `missing_template`, `unavailable_external_template`, plus stale-index reasons). `xray_definitions` adds `selectorState`, `selectorStateReason`, `templateSource`, `templateState`, `templatePath`, and `templateStateReason` while retaining `selector` and `templateChildren`.

- **Added recoverable cursor pagination to `xray_grep`.** All token, substring, phrase, lineRegex, files-only, and invert results now page after final stable ranking (and after substring-OR auto-balance) but before preview formatting; count-only remains global and cursor-free, byte fitting advances by complete files actually returned, and oversized line details degrade to file metadata before an irreducible-item error. Files-only queries skip preview rereads and therefore do not emit stale-line warnings.

- **TypeScript duplicate roots now honor the explicit legacy ambiguity policy.** `xray_callers` continues to report bounded `ambiguous_typescript_root` candidates by default; `ambiguityPolicy="legacy"` restores historical name-based fan-out for single and batch UP/DOWN queries while marking the response partial and unsafe for exact semantics.

- **Prevented TypeScript bare calls from resolving by repository-wide name coincidence.** Definition indexing now tracks lexical scopes, exact local and same-file callable targets, shadowing, closure capture, destructuring, and conservative reassignment; parameters, unresolved locals, imports awaiting module resolution, and unknown globals remain typed unresolved references that reduce semantic completeness without polluting the exact call tree. Nested and module-private callables are visible to `xray_definitions` as `function` entries with `local` modifiers, owning-class context where applicable, and AST-derived signatures for exact upward/downward attribution. Name-only caller queries now stop with bounded `ambiguous_typescript_root` candidates instead of merging multiple exported, class-owned, or private roots; elected root identities also control `rootMethod`. TypeScript AST recursion now stops at a Windows-safe language-specific depth cap. The definition-index format changes and existing indexes rebuild automatically.
...
- **Single-term exact token searches now add a normalized file-stem signal to TF-IDF ranking.** CLI `xray grep --exact` and MCP `substring=false` use the same additive pseudo-TF when a term matches the file stem after case-folding and separator removal, so `OrderProcessor` promotes `order_processor.rs` without changing substring, phrase, line-regex, token-regex, or multi-term modes. On the checked 40-query corpus, overall NDCG@10 rises from 0.906323 to 0.914894; the exact-identifier class rises from 0.81254 to 0.88111 while Recall@50 remains 1.0.
- **Added a checked `xray_grep` relevance evaluation harness.** A versioned 18-file neutral corpus and 40 graded queries now measure NDCG@10 and MRR@10 ranking quality, grade-3 Success@1, and a Recall@50 matching canary through the real MCP dispatch path. CI checks all-query and 35-query scorer-only aggregates, per-class quality, query-level rankings, and corpus bytes/extensions against a deterministic production-scoring baseline; an explicit ignored test writes machine-local latency and query details under `target/relevance/`.

- **Relevance evaluation now supports intent-aware blind candidate grading.** Query records require an explicit intent, the ignored collector calls production `xray_grep` at depth 50 while omitting ranks and scores, and schema-v3 quality reports include `explicitNegativeHitsAt10`. Candidate reports use an independent schema v1. Production ranking is unchanged.

- **`xray_grep` now ignores duplicate search terms instead of treating them as hidden ranking boosts.** Case-equivalent token, substring, phrase, and token-regex terms are deduplicated while line-regex patterns use exact-string identity, preserving meaningful case and whitespace differences. Duplicate inputs no longer inflate scores or occurrence counts, distort `mode=and`, or change `autoBalance` caps; CLI `xray grep --exact` now applies the same stable deduplication, and the first unique spelling remains in diagnostics. If phrase deduplication leaves one effective phrase, `searchMode` reports `phrase` rather than a multi-phrase mode.
...
- **`xray_git_*` tools now report a missing git binary and a non-repository path in one consistent, actionable form.** Both helpers behind the git tools — the one used by `xray_git_history`, `xray_git_diff`, `xray_git_authors`, `xray_git_activity` and `xray_git_blame`, and the separate one used by `xray_branch_status` — now share the same two messages: `git is not available: <io error>. Install git and make sure it is in PATH.` and `Not a git repository: '<repo>'. xray_git_* tools need the 'repo' argument to point at a git working tree.` Previously the wording depended on which of the two a tool happened to use — `xray_git_history` answered `Failed to execute git: program not found. Is git installed and in PATH?` while `xray_branch_status` answered `Failed to get current branch: Failed to run git: program not found` — and a wrong `repo` argument leaked raw `fatal: not a git repository (or any of the parent directories): .git` with no hint about which argument to fix. A `repo` path that does not exist is no longer misreported as a missing git installation: the spawn fails with the same error kind either way, so the path is probed before the message is chosen, and a spawn failure that is conclusive about neither — a permission error, say — is reported neutrally as `Failed to start git in '<repo>': <io error>`. The non-repository diagnosis no longer relies on git's English wording alone: the stderr match is anchored to the `fatal:` line so an ordinary diagnostic quoting a user-supplied path cannot trigger it, and a filesystem probe covers localized git builds while still treating bare repositories, linked worktrees and a `GIT_DIR` override as repositories. That probe only runs on git's fatal exit code 128, so a routine non-zero status such as `git config --get-regexp` reporting an unset key never pays for it. Every other git failure keeps its stderr and now names the failing subcommand instead of the generic `git command failed`. Nothing else changes: no git invocation, argument or environment variable is altered, the tools still answer with `isError: true` and the message in the response's `error` field, the background git-cache builder keeps its own log-only diagnostics, and a directory without `.git` still starts the server normally and only skips that cache.

- **`xray_edit` now controls line endings through a `lineEnding` parameter (`preserve` | `auto` | `lf` | `crlf`).** Previously a new file was always written with LF and there was no way to ask for CRLF at all — passing `\r\n` inside `content` did not help, because input is parsed into logical lines and re-serialized with the format the tool picked. In a Windows repository running `text=auto`, `core.autocrlf=true` and `core.safecrlf=true` this made `git add` fail with `fatal: LF would be replaced by CRLF`, forcing a `git -c core.safecrlf=false` workaround.

  An existing file still keeps its detected format when the parameter is omitted, and `preserve` is rejected for a file that does not exist yet. A file being created now defaults to `auto`, which resolves the line ending Git would materialize in the working tree — only through Git, and only for the physical destination, so a directory symlink attributes the file to the repository it actually points into. An effective `eol=lf`/`eol=crlf` attribute wins outright; otherwise `text`/`text=auto` combined with `core.autocrlf`, then `core.eol`, then the platform default decides; an unspecified `text` is governed by `core.autocrlf` alone. The legacy `crlf`, `-crlf` and `crlf=input` attributes are honored per the gitattributes(5) compatibility table, which `git check-attr text eol` does not surface — but only when neither a modern `text` nor a modern `eol` attribute applies, which is what Git itself does. `.editorconfig` and the format of neighbouring files are deliberately not consulted: neither affects whether `git add` succeeds, and both could contradict Git and reintroduce the failure. No Git process is spawned when there is no `.git` above the path, and a missing Git, a path outside a worktree, or any Git failure degrades to LF — the previous behavior. `xray_edit` still never runs `git add` and never writes Git config.

  Converting an existing file counts as a real change — the file is rewritten, `resultHash` differs from `sourceHash`, and the diff reads `(line endings: LF -> CRLF)` when the text itself is unchanged; a conversion with no text edit is `operations: []` plus `lineEnding`. Responses (including `dryRun`) add `requestedLineEnding`, `originalLineEnding`, `resultLineEnding`, `lineEndingDecisionSource` (`explicit` | `preserve` | `default-preserve` | `gitattributes-eol` | `git-worktree-policy` | `fallback-lf`) and `lineEndingChanged`, with `lineEnding` kept as an alias of `resultLineEnding`. The parameter is resolved per file for `paths`, mixed LF/CRLF files are still rejected before any write, and a file with no line terminators — byte-identical under either format — reports `originalLineEnding: null` and `lineEndingChanged: false` instead of claiming a conversion.

- **`xray_edit` now rejects a NUL byte in text a request asks to write.** `operations[].content`, `edits[].replace` and `edits[].content` are checked before anything is applied. Bytes already present in the file are untouched, so no previously-accepted edit starts failing. The tool has always refused to read a file whose first 8 KB contain a NUL, so writing one produced a file it could not edit again; a NUL would also flip Git's `text=auto` detection to binary and invalidate the line ending `auto` resolved.

- **Audited and hardened the two review agents.** `xray-code-reviewer` gained a read-only command whitelist so it acquires the diff itself instead of trusting whatever patch the requester pasted, and its project profile no longer claims the crate has no FFI and no production `unsafe`; the evidence protocol (evidence tiers, the `xray_callers` false-negative trap, validation delegation, confidence calibration), a diff budget rule for large PRs, a security check section, and repository gates are back. `xray-script-reviewer` replaces stale suite counts — it claimed 6 fixtures and 27 checks against an actual 7 and 38 — with a map of all nine `scripts/mcp-filter/` suites, corrects the mutated-paths profile to `<git-common-dir>/info/attributes` and both installed filters, adds hard execution-safety limits for the `execute` tool, and binds the public entry-point matrix to the tests that actually cover it. `prompts/rust-expert-code-review-prompt.md` was removed as a competing source of truth that had already drifted from `Cargo.toml`.
...
- **D20 C# semantic scale baselines are reproducible.** `scripts/bench-d20-csharp-semantics.ps1` deterministically generates 100,000-method overload and namespace-collision corpora from seed `0xD20C5`, validates the committed corpus manifest, runs release `def-index`/MCP measurements with cold-build, RSS, index-size, incremental-lifecycle, candidate-set, and warm-query samples, and stores machine-specific raw output under `target/`. A run records a baseline without claiming budget compliance until a comparator with matching corpus, lockfile, toolchain, hardware, and environment is supplied.
...
- **D20-Core now has a standalone release-binary gate.** `scripts/test-d20-release.ps1` builds or accepts a candidate binary, compiles and runs an independent .NET fixture oracle, exercises exact and ambiguous C# calls through MCP, verifies watcher lifecycle cleanup, and emits raw JSON measurements under `target/`. The optional baseline control builds pre-D20 commit `ef14a4875914fb8a5323313001f06c246688513c` with the same lock/toolchain, requires the old overload/namespace behavior to fail, and uses its genuine v6 index to verify the current v6-to-v7 rebuild path. Migration checks poll `xray_info` until asynchronous definition rebuilds are ready instead of racing `INDEX_BUILDING`, and record readiness attempts and wall time.
...
- **Valid token-regex queries with an empty resolved file scope now skip vocabulary expansion and posting traversal.** Regex compilation still runs first, so invalid patterns remain errors. `regexExpansion.schemaVersion=2` is a semantic migration: clients must use `accountingScope` before interpreting counters or previews (`none` for the empty execution universe, `globalVocabulary` for the existing non-empty path). Bounded phase timings separate planning, compilation, universe construction, scan/collection, sort/dedup, total expansion, and posting scoring. No on-disk schema change or reindex is required; the `token_regex_empty_scope` and `token_regex_phase` Criterion groups cover 1k, 10k, and 50k vocabularies.

- **Narrow token-regex file scopes can expand against authoritative `file_tokens`.** Scoped execution reports `accountingScope=resolvedFiles` and does not scan the global vocabulary merely to recover legacy metadata. The `termsMatched` numerator remains result semantics, while its denominator follows the selected execution universe and may differ from global fallback when matching tokens exist only outside the resolved scope. Clients must interpret the denominator with `accountingScope`; `globalMatchedTokenCountKnown=false` makes the unavailable global total explicit. A measured 50% scope-reference threshold was selected after comparing 10%/25%/50% across 1k/10k/50k vocabularies and low/medium/hot sharing. The `token_regex_scope_strategy` and `token_regex_scope_threshold` Criterion groups cover forced global, forced scoped, and planner-selected execution. Search paths, occurrences, scores, ranking, and on-disk schema remain unchanged.
...
- **`xray_edit` now detects concurrent source changes before atomic commit.** Existing files are revalidated by physical identity, final path kind, hard-link count, supported metadata, and raw SHA-256 immediately before replacement; new files atomically reserve the destination name and revalidate symlinked parents. Hard-link-capable filesystems publish complete staged bytes immediately; other filesystems fall back to portable create-new copy without clobbering an existing destination, so the new file may be visible while it is filled. Responses expose `sourceHash` (`null` for a new file) and `resultHash`, while optional single-file `expectedHash` supports round-trip preconditions. Multi-file edits preflight the full batch and recheck each target before commit; an external change between commits can still produce the existing, explicitly reported partial-commit outcome.
...
- **Windows definition-index updates now share the content index's case-insensitive path identity.** Definition build, edit, watcher, reconcile, caller lookup, and removal paths reuse one internal key for case-variant spellings. The persisted definition-index format advances to version 5 so old keys are rebuilt from disk.

- **Incremental indexing now preserves normal ignore scope.** Files excluded from every active index by `.ignore`, `.gitignore`, global gitignore, or enabled `.git/info/exclude` rules are not added by either `xray_edit`'s synchronous refresh or the debounced filesystem watcher; edits are still written and return `reindexStatus: "skipped"` with `reindexSkipReason: "ignoredByIndexRules"`. Hidden paths preserve the existing split scope: content and file-list indexing skip them, while an active definition index is refreshed synchronously and by the watcher.
...
- **Exact phrase search now preserves one-character words.** Indexed tokens still narrow candidate files and lines, but the final match verifies the complete raw phrase, preventing false positives such as treating `Xray z` as `Xray`.
- **Windows content-index updates now use case-insensitive path identity.** Watcher, synchronous reindex, and reconciliation reuse the existing `file_id` for case-only path variants; watch-index initialization also removes previously duplicated IDs and stale postings.
- **Directory-scoped grep no longer canonicalizes every indexed path.** Scope validation retains its workspace-boundary checks, while trusted indexed paths use lexical prefix matching; large-repository `file + dir` and `scope_not_found` queries return in milliseconds instead of seconds. Token-regex guidance now correctly states that `^` and `$` anchor indexed tokens, not source lines.
...
- **`xray_grep`: stale line evidence is never returned, and phrase search
  recovers matches moved by on-disk edits.** The inverted index records physical
  line numbers captured at index time, while result context is read from the
  current file. Substring and exact-token searches verify posting lines against
  fresh content and drop stale entries from both `lineContent` and `files[].lines`.
  Phrase search now rescans the already-loaded candidate content when indexed
  lines no longer describe the current phrase locations, without a second disk
  read. Recovered matches return their current lines; removed matches no longer
  produce a trustworthy `not_found`. Detected staleness sets `resultStatus` to
  `partial` with reason `stale_index` and `totalKnown=false`, while
  `phraseDetail` reports stale, recovered, and unresolved unique file counts.
  Multi-phrase diagnostics aggregate across every phrase instead of exposing only
  the last one. Unreadable candidates and worker failures now also produce partial
  results with `file_read_error` or `worker_panic`. The regression matrix covers
  line insertion/deletion, removed phrases, count-only, multiple occurrences,
  punctuation, multi-phrase OR/AND, truncation, read failures, and worker failure.
...
- **`xray_branch_status`: `behindMain` no longer reports a bogus count when a repo carries an unrelated `main`/`master` orphan ref.** `detect_main_branch_name` resolves the trunk by a fixed `main → master` priority and `compute_behind_ahead` then measured `HEAD…origin/main` unconditionally. In a repo whose real trunk is `master` but which also carries an unrelated `main` (stale GitHub default, migration leftover, orphan branch), HEAD and `origin/main` share no merge-base, so the symmetric-difference count blew up to tens of thousands (`behindMain: 65028`) — a number that never shrank on `git pull` because pulling `master` does not change the distance to an unrelated `origin/main`. **Fix (two defects):** (a) behind/ahead is now computed against the trunk remote that actually *shares history* with HEAD — the detected trunk is tried first, then the alternate well-known trunk (`main`↔`master`), so a repo on `master` with an orphan `main` correctly reports against `origin/master`, and the trunk actually compared against is surfaced in `mainBranch`; (b) when NO well-known trunk remote shares a merge-base with HEAD the count is suppressed (`null`); a new `unrelatedHistories: true` field + explanatory `warning` are emitted **only with positive evidence** of a genuine orphan — a trunk remote exists, the repo is not shallow, and HEAD resolves to a commit. A shallow graft (which can truncate the real common ancestor) or an unborn HEAD suppresses the count *without* the orphan claim, so a shallow CI checkout is never mislabelled. Pure git-CLI: one `git merge-base HEAD origin/<trunk>` guard per candidate, plus `rev-parse --verify` existence probes only on the fallback path — the common single-trunk repo pays one extra `merge-base` spawn. **Tests:** `test_branch_status_falls_back_to_related_trunk_when_main_unrelated`, `test_branch_status_suppresses_count_for_unrelated_histories`, `test_branch_status_shallow_missing_merge_base_not_flagged_unrelated`, `test_resolve_behind_ahead_unborn_head_not_unrelated`, and `test_build_warning_unrelated_histories`, built on real temp repos using `git commit-tree` parentless root commits to synthesise unrelated histories; existing `detect_main_branch_name` / `build_warning` suites updated for the new signature; full `cargo test --bin xray` 2693 pass, clippy clean.
...
- **`xray_definitions regex=true` is now case-sensitive (behaviour change).**
  ...
  (`^Fetch[A-Z][a-z]+Async$` matched `FetchWithRetryLogicAsync`).
...
- **`xray_fast` glob patterns ending in `*` or `?` now match files with
  ...
  produced fully-anchored `^…$`, so `Invoice?_V?` could never match
  `Invoices_V2.sql` (length-exact rejection). The trailing `$` anchor
  ...
  `Invoice?_V?` matches `Invoices_V2.sql`; `Order*` matches both
...
- **`xray_callers` — mid-tree interface expansion + de-noised `interface_vias_caveat` (one PR, two related fixes).** Pre-fix, `expand_interface_callers` ran ONLY at `current_depth == 0`, so callers reachable through interfaces of intermediate tree nodes were silently dropped — even with the default `resolveInterfaces=true`. Investigations with chains of DI-injected interface layers had to issue follow-up `xray_callers class=I<X>` calls per intermediate class to recover the missing branches. The condition is now `current_depth == 0 || (parent_class implements an interface declaring this method)` — depth>0 expansion is gated by a new shared helper `interfaces_declaring_method(class, method, def_idx)` that returns only interfaces from `class.base_types` that actually declare a method with the searched name on the interface itself. The gate is intentional: without it, every node would pay the cost of scanning `name_index[method_lower]` inside `expand_interface_callers` for popular method names (`Dispose`, `Equals`, `ToString`). Mid-tree `expand_interface_callers` calls now recurse with `current_depth` (NOT `current_depth + 1` and NOT the previously-hardcoded `1`) — the interface walk is a *sibling* search whose results are appended at the same visible tree level as direct callers, so consuming an extra depth level would silently drop nodes when the visible tree level equals `max_depth`. This also fixes a pre-existing depth-0 off-by-one (root-level interface-walk branches used to consume one extra depth level relative to direct-caller branches). Per-builder dedup `iface_expand:<iface>:<method>` in `self.visited` prevents re-expanding the same (class, method) pair across multiple branches. Per-builder cache `interface_lookup_cache: HashMap<(class_lower, method_lower), Vec<String>>` keeps the gate at O(unique pairs) instead of O(nodes). The same `interfaces_declaring_method` helper now also gates `interface_vias_caveat`, fixing a latent false-positive: previously, `Foo : IDisposable, IMyThing` with a query for `DoWork` would surface `class=IDisposable` as a recommended re-run target, even though `IDisposable` does not declare `DoWork`. Hint wording updated to "interface(s) declaring `<method>`" to reflect the tighter semantics. Tests in `src/mcp/handlers/advisory_hints.rs`: 5 new unit tests for `interfaces_declaring_method` (no-interface class, IDisposable-only base + unrelated method, single match, deterministic sort across multiple matches, ambiguous-short-name union); new `interface_vias_skips_interfaces_without_matching_method` (regression for the false `class=IDisposable` recommendation); existing `interface_vias_*` tests updated to wire interface methods into their fixtures (the gate was already implicit in the real index since AST captures interface method bodies; tests now match production shape). Tests in `src/mcp/handlers/callers_tests.rs`: `test_advisory_interface_vias_emitted_when_class_implements_interface` updated to add an interface method def so the new gate is satisfied (and `base_type_index` index-shift documented); new end-to-end `test_mid_tree_iface_expansion_finds_caller_through_sibling_impl` builds a `Worker.Foo ← Impl1.Mid ← (IMid sibling-impl walk) → Impl2.Mid ← Consumer.Run` chain and pins three contracts in one test: (a) `Consumer.Run` surfaces at depth 2 with `resolveInterfaces=true` AND `max_depth=2` (this assertion fails if the recursive depth handoff is reverted to `current_depth + 1`), (b) `Consumer.Run` is absent with `resolveInterfaces=false` (no interface walk at all), (c) `Consumer.Run` is absent with `max_depth=1` (depth budget exhausted before expansion site). Companion `test_root_iface_expansion_honors_visible_depth_budget` independently locks the depth-0 (root-level) branch of the same recurse fix: fixture `ConcreteImpl1.Method (no direct callers) ← (root IFoo→ConcreteImpl2 walk) ← Caller.Run ← RootCaller.Top` with `max_depth=2`; asserts `RootCaller.Top` at visible depth 2 (fails under the same `current_depth + 1` mutation). All 2624 tests pass.

## 0.2.6 (2026-05-11)

- **`xray_definitions` schema guidance for optional filters and `containsLine`.** The `attribute` and `baseType` parameter descriptions now explicitly tell MCP clients to omit unset optional strings instead of passing empty strings. `containsLine` now advertises its `>= 1` line-number requirement, the need for a real `file` path, and the fact that it switches the request into line-containment lookup. Runtime validation and query semantics are unchanged.
...
- **`xray_git_history` / `xray_git_authors` / `xray_git_activity` — HEAD-pinned cache invalidation for empty results (correctness fix).** The in-memory git history cache used to return cached empty results indefinitely, even after the repo HEAD moved. A query for a file that was added (or first matched a new author/message/date filter) AFTER the cache was built returned a stale empty result, silently producing a false negative ("file has no history" / "no commits matched"). Empty cache results are now HEAD-pinned: when the cache returns zero rows AND the repo's live HEAD differs from the cache's snapshot HEAD, the handler falls through to the authoritative `git log` CLI fallback. Non-empty cache results are unchanged (the cache is monotonic for files already known) and the extra `git rev-parse HEAD` call is paid only on empty results, so non-empty queries see zero overhead. The `xray_git_activity` `includeDeleted=true` post-filter is unaffected: a known path that the cache populates and `includeDeleted` then trims to zero is still served from cache. Cache `read()` lock is now released before any git subprocess (`rev-parse HEAD`, `ls-files`) to avoid blocking concurrent cache rebuilds.

- **`scripts/setup-xray.ps1` hardening — tracked compact MCP configs now fail closed instead of reporting false success.** A tracked `.vscode/mcp.json` shaped as `{"servers":{}}` cannot be safely round-tripped by the line-based smudge/clean filter without changing the canonical tracked formatting. Setup now treats “filter installed but snapshot injection failed” as a hard error: it rolls back filter artifacts and exits non-zero instead of completing with no registered xray server. `scripts/mcp-filter/test-vscode-tracked.ps1` now covers this compact tracked case.

- **`scripts/setup-xray.ps1` follow-up hotfix — local git protection now survives repo `.gitignore` re-include rules.** Some repositories ignore `.vscode/*` but explicitly re-include `!**/.vscode/mcp.json` in tracked `.gitignore`. That higher-priority negation overrides `.git/info/exclude`, so the installer could correctly append `.vscode/mcp.json` to `.git/info/exclude` and Git would still show it as untracked. After writing the exclude entry, setup now verifies the effective ignore result with `git check-ignore`; if a repo `.gitignore` keeps `.vscode/mcp.json` visible, setup creates a local untracked `.vscode/.gitignore` containing `mcp.json` (and hides that helper too). If the helper itself is re-included, setup now fails closed, removes freshly generated files, and rolls back exclude lines added by the failed run. The standalone installer regression suite now covers this Shared-style `.gitignore` precedence case on PS 7 + PS 5.1. For root `.mcp.json`, where no closer per-directory override exists, setup now fails closed, removes a freshly generated file, and rolls back its exclude line if git protection is ineffective.

- **`scripts/setup-xray.ps1` follow-up hotfix — rerun after a partial failed install now restores git protection.** If the previous broken in-memory launch had already created an untracked `.vscode/mcp.json` with an `xray` entry, a rerun could warn that the entry already existed, accept the default "do not update" answer, and skip the final git-protection path because `$writeVscode` had been set to `$false`. The installer now tracks this as a recovery case: it leaves the existing config unchanged but still adds untracked generated MCP configs to `.git/info/exclude`, so `git status` stays clean. The same recovery flag exists for Copilot CLI `.mcp.json`. This follow-up also switches git-repo detection from `Test-Path <repo>/.git` to `git -C <repo> rev-parse --show-toplevel` and normalizes to the real worktree root before writing configs, so non-standard worktree layouts do not skip git protection.

- **`scripts/setup-xray.ps1` hotfix — documented in-memory bootstrap no longer crashes before git protection.** The README A1 launch form (`& ([scriptblock]::Create((Invoke-WebRequest ...).Content))`) runs the script body from memory, where `$MyInvocation.MyCommand.Path` is `$null`. The merged `perf/setup-xray-scan-prune` path called `Split-Path -Parent $null` while configuring the MCP filter and aborted after creating `.vscode/mcp.json` but before the final git-protection block could add untracked generated configs to `.git/info/exclude`. The hotfix null-guards both filter call sites and lets `Install-McpFilter` accept `$null` / empty `-ScriptDir`, forcing the embedded filter fallback. `scripts/mcp-filter/test-standalone-install.ps1` now exercises file-based, in-memory, partial-rerun recovery, and repo `.gitignore` negation setup on PS 7 + PS 5.1, enables both VS Code and Copilot CLI in the fresh install paths, and asserts `.vscode/mcp.json` stays hidden with `git status --short` clean even when `.git/info/exclude` is overridden by a tracked `.gitignore` rule.
...
- **`scripts/setup-xray.ps1` — four production install-path fixes
  ...
  guard these fixes:
    ...
    * `scripts/mcp-filter/test-standalone-install.ps1` (100 assertions
      across PS 7 + PS 5.1 file-based, in-memory, partial-rerun,
      `.gitignore` negation, and fail-closed cleanup rounds) — stages
...
- **XML config discovery hints — close the `xray_grep ext=["xml"]` 0-results bypass.** Fixes the user-story scenario where an agent narrowed `xray_grep` by `ext=["xml"]`, got 0 matches because the workspace uses `.manifestxml`/`.config`/`.csproj`/etc., and fell back to PowerShell `[xml]` parsing instead of `xray_definitions` on-demand. Three independent surfaces patched, each with its own regression test:
  ...
  - **`xray_definitions` glob detection (`xml_on_demand.rs::try_intercept`):** `file=["*.xml"]`/`file=["q?.xml"]`/`file=["[abc].xml"]` patterns are now rejected with a typed actionable error pointing at `xray_fast pattern=["<glob>"]` BEFORE the Windows `canonicalize()` call. Previously the request bubbled up `os error 123` (ERROR_INVALID_NAME) — opaque and non-recoverable for the agent.
